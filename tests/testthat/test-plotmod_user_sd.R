...
w_lo_chk <- par_t_chk[26, "est"] - 1.5 * sqrt(par_t_chk[18, "est"])
w_hi_chk <- par_t_chk[26, "est"] + 1.5 * sqrt(par_t_chk[18, "est"])
...
a_hi_chk <- 1.25 * (coef(fit)["m2~x2"] + coef(fit)["m2~x1x2"] * ov_means["x1"]) *
             ov_sds["x2"] / ov_sds["m2"]
a_lo_chk <- -1 * a_hi_chk
...
b_lo_chk <- b_x_std - 1.25 * b_xw_std
b_hi_chk <- b_x_std + 1.25 * b_xw_std
chk <- c(a_lo_chk, b_lo_chk, a_hi_chk, b_hi_chk)
names(chk) <- NULL
